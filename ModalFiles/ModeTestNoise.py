...
def compute_time_derivative_sg(a, dt, window_length=45, polyorder=3):
    da = savgol_filter(
        a,
...
a_dot_train = compute_time_derivative_sg(a_train, dt)
v_train = a_dot_train
v_dot_train = compute_time_derivative_sg(a_dot_train, dt)
...
if METHOD == "STRidge":
    ...
    opt = FROLS(max_iter=1, alpha=0, kappa=1e-5)
...
# Initial condition for SINDy
a0 = a[0]
v0 = compute_time_derivative_sg(a, dt)[0]
...
if PLOT_MODES:
    for mode in range(N_MODES):
        ...
        ax.plot(t[:N_train], a_sim_real[:N_train, mode], '--',
                color='steelblue', label="SINDy (train)")
        ax.plot(t[N_train:], a_sim_real[N_train:, mode], '--',
                color='tomato', label="SINDy (forecast)")
...
if PLOT_AMPLITUDES:
    plt.imshow(modes.real[:, :1000].T, aspect='auto', origin='lower')
    ...
    plt.imshow(modes.imag[:, :1000].T, aspect='auto', origin='lower')
    plt.xlabel("Time index")
    plt.ylabel("Mode number")
...
a_test = np.hstack([a_complex_test.real, a_complex_test.imag])

a0_test = a_test[0]
v0_test = compute_time_derivative_sg(a_test, dt_test)[0]
...
a_sim_complex_test = (
    a_sim_test[:, :N_MODES] + 1j * a_sim_test[:, N_MODES:]
)
...
U_hat_true_test[:, 1:N_MODES + 1] = modes_test[:, 1:N_MODES + 1]
